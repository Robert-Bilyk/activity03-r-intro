...
The errors told me right where the error occurs in the code. In the first issue it was a missing comma while the second one needed single quotes surrounding it instead.
...
Cbind was turning it into a matrix so I just changed the command to simpyl create a new vector.
...
Dataframe needed a period between the data and frame area.
...
Because it's a dataframe you need to use nrow instead of length to answer.
...
Needs a dollar sign instead of a % symbol.
...
```{r}
codematrix <-matrix(step4, ncol=5)
even <- matrix(seq(2,502,2))
codematrix[,1]<- codematrix[,1]-even
codematrix[18:24,3]<- codematrix[18:24,3]-100
codematrix[,4:5]<- codematrix[,4:5]*2
vector1 <- c(codematrix)
```
...
```{r}
vector1[38:465] <- vector1[38:465]^.5
vector1 <-round(vector1,0)
vector1[vector1==39] <- 20
vector1
```
...
```{r}
stringr::str_c(my_symbols$Symbol[vector1], collapse = "")

```

Google the first line of this message, if you do not recognize it, to see what it is.

very difficult but I did understand everything

Comment on what you noticed about the activity as a whole.

it was mostly just modifying and performing different operations on a vector
